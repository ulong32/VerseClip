...
if (location.pathname === "/mypage/myphoto/") {
    const photoElems = document.querySelectorAll("a.photo_Anchor");
...
<div id="dlAnnounce" style="border-radius: 20px; position: fixed; right:50px; bottom: 50px; padding: 25px; background-color: white; box-shadow: 0 0 6px 6px #0002">
    ...
    `);
        photoElems.forEach((element, index) => {
            fetch(element.href)
                ...
                .then((doc) => {
                    let photoDetailElm = doc.querySelector(".detail_thumbImage");
                    setTimeout(() => {
                        download(
                            photoDetailElm.src,
    ...
    alert("アイプリ マイキャラページの「アルバム」画面にて実行してください。");
}
